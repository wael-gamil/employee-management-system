...
export function useSettingsStore() {
  ...
  const loadSettings = async () => {
    ...
    } else {      settings.value = getDefaultSettings()
      
      // Add notification for first-time setup
      notificationStore.addNotification('SETTINGS_LOADED', 'System settings initialized with default values');
    }
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, 500))        // Add notification and toast for settings save
        notificationStore.addNotification('SETTINGS_UPDATED', 'System settings have been updated');
  ...
  const applyInterfaceChanges = async (key, value) => {
    ...
    switch (key) {
      case 'theme':
        ...
        notificationStore.addNotification('THEME_CHANGED', `Theme changed to ${value}`);
        break
    ...
    switch (key) {
      ...
      case 'maintenanceMode':
        if (value) {
          ...
          notificationStore.addNotification('MAINTENANCE_MODE', 'Maintenance mode has been enabled');
        } else {
          console.log('Maintenance mode disabled')
          
          // Add maintenance mode disabled notification
          notificationStore.addNotification('MAINTENANCE_MODE', 'Maintenance mode has been disabled');
        }
        break
  ...
  const exportSettings = () => {
    ...
    link.href = url
    link.download = `ems-settings-${new Date().toISOString().split('T')[0]}.json`
  ...
  const createBackup = async () => {
    ...
    try {
      ...
      const link = document.createElement('a')
      link.href = url
      link.download = `ems-backup-${new Date().toISOString().split('T')[0]}.json`
      document.body.appendChild(link)
      link.click()
      document.body.removeChild(link)
      URL.revokeObjectURL(url)
  ...
  const addUser = async (userData) => {
    ...
    notificationStore.addNotification('USER_CREATED', `New user "${newUser.name}" has been created`);
      toastStore.addToast({
        type: 'success',
        title: 'User Created',
  ...
  const updateUser = async (userId, updates) => {
    ...
    // Add notification and toast
    notificationStore.addNotification('USER_UPDATED', `User "${updatedUser.name}" has been updated`);
      toastStore.addToast({
        type: 'success',
        title: 'User Updated',
  ...
  const deleteUser = async (userId) => {
    ...
    if (success && user) {
      // Add notification and toast
      notificationStore.addNotification('USER_DELETED', `User "${user.name}" has been deleted`);
        toastStore.addToast({
          type: 'success',
          title: 'User Deleted',
